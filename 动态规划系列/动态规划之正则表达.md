...
>  从右到左看，i 为pattern，j为text：
> 
>  `.` 匹配 或相等匹配，[i] ==[j] or . 且 dp[i-1][j-1]
> 
> `*` 匹配， 零个： dp[i-2][j] 注意i-2为-1时(所以sz+1 且要判断)； 一个及多个： pattern[i-1]匹配text[j], 且dp[i][j-1]
> 
> dp size n+1 columns * m+1 rows
> 
> ```python
        ...
        for i in range(1, m+1):
            for j in range(n+1):
                if j and ((p[i-1] == s[j-1]) or (p[i-1] == '.')):
                    ...
                    dp[i][j] = (i>1 and dp[i-2][j]) or (j>0 and dp[i][j-1] and i>1 and (s[j-1]==p[i-2] or p[i-2] == '.'))
        return dp[m][n]
> 
> ```

> 下文安装top down的方法则是从左朝右看 使用memorization
