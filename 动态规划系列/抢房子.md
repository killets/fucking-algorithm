...
> 变量就1个，房子i，选择就是当前房子偷不偷；偷则，当前+前2个房子的最大，不偷则前1个房子的最大
> 
> 初始化：dp[0]第一个房子自己, dp[1] max（第一个，第二个房子）
...
> 我的思路 divide & conqure：base return（抢， 不抢），根据左，右子树的解来求解整个树的解。
> 
> max （抢父节点，不抢）
> 
> max { 父节点+左不抢+右不抢， max（左抢，左不抢）+ max(右抢，右不抢）}
