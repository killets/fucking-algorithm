...
遇到后面的符号才知道前面num搞玩了，这时候才把前面的sign和num存进去。
更新sign，num重新来。
+-*／ 前面可以是任何符号，这里定义push这个函数把前面的数存进去。
最后stack里面存的都是带符号的数

int(num1/ num2)
而不是 num1//nums2

想搞清楚一个问题，py2和py3是否都是朝一个方向（远离0的方向）
>>> -3/2.0
-1.5
>>> int(-3/2.0)
-1
...
结果是一样的，要想truncate toward 0, 则用 int(num1/num2)