...
> 我觉得这里bt做法是错误的，会导致非常慢. 走过了的还会重复走
> 可以先都设为-1， 最后再单独用个循环遍历把-1改为new color，不管color是否和original一致。
...
> 要么周围有越界，要么周围有其他颜色，这样的点才是边界点。周围4个方向... 即都得在界限内且为相同颜色，那么从这个点出发，4个点都返回1（边界内同色）或0（越界或异色），只要有一个0就为边界点。引申为sum < 4.
