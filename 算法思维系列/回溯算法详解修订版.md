...
> DFS mark visited是真正执行的时候, for 外面
> BT 因为要unset，放在了for内部， for do (visited ) next undo (unvisit)

**其核心就是 for 循环里面的递归，在递归调用之前「做选择」，在递归调用之后「撤销选择」**，特别简单。

什么叫做选择和撤销选择呢，这个框架的底层原理是什么呢？下面我们就通过「全排列」这个问题来解开之前的疑惑，详细探究一下其中的奥妙！

### 一、[全排列问题](https://leetcode.com/problems/permutations/) 
...
```
> TODO
> 这让我想到了wordbreak ii, 那题为什么要用memorization，全排列为啥不用？ 
> 为什么说全排列没有重叠子问题?
> 我觉得是有的，比如 1，2，3，4，5全排列
> 拿1，拿2，剩下345；拿2，拿1，剩下345. 345是重叠子问题
> 那为什么不用mem？或dp 
> 感觉原因是因为重叠的占比太小了, 每一步可以拿出很多可能
```
...
**有的时候，我们并不想得到所有合法的答案，只想要一个答案，怎么办呢**？[gjnote: check] 比如解数独的算法，找所有解法复杂度太高，只要找到一种解法就可以。
...
> if (backtrack(board, row + 1))
            return true;
> ```
> 或者 self.found = True,  然后进入开头时判断 if self.found: return
> 
> 因为没有back，所以数组会被改变
...
>划重点：其实想想看，回溯算法和动态规划是不是有点像呢？我们在动态规划系列文章中多次强调，动态规划的三个需要明确的点就是「状态」「选择」和「base case」，是不是就对应着走过的「路径」，当前的「选择列表」和「结束条件」？
>
>而今天的两个问题，都没有重叠子问题，也就是回溯算法问题了
