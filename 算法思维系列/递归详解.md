...
```c++
// 法1: divide and conqure
// 法2: range sum
class Solution {
public:
    int pathSum(TreeNode* root, int sum) {
        unordered_map<int, int> sumfrq;
        sumfrq[0] = 1;
        int total = 0;
        dfs(root, 0, sum, sumfrq, total);
        return total;
    }
    
    void dfs(TreeNode* p, int runningsum, int sum, unordered_map<int, int>& sumfrq, int& total){
        if(!p) return;
        runningsum += p->val;
        if(sumfrq.count(runningsum - sum)){
            total += sumfrq[runningsum - sum];
        }
        sumfrq[runningsum]++;
        if(p->left) dfs(p->left, runningsum, sum, sumfrq, total);
        if(p->right) dfs(p->right, runningsum, sum, sumfrq, total);
        sumfrq[runningsum]--;
    }
