我最近在 LeetCode 上做到两道非常有意思的题目，382 和 [398](https://leetcode.com/problems/random-pick-index/) 题，关于水塘抽样算法（Reservoir Sampling），本质上是一种随机概率算法，解法应该说会者不难，难者不会。
...
```
Reservoir Sampling
由结论推出／猜测出规则：最后都是1/n，然后想到拆解为 1/2 * 2/3 * 3/4 ... 这样的项乘积
一句话： 用当前新数替换的概率为1/i; 否则就保留旧的res(前面某一位的数)。
for index i from 2nd number to last number
change possibility (use num[i] to replace) : 1/i
keep old possibility: i-1/i
so for every element:
   1/i * i/(i+1) * ... * (n-1)/n = 1/n   从自己替代旧的开始才有意义，所以第一项是1/i
```

找target相同的,返回值是index
